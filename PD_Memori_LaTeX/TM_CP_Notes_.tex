...
\begin{enumerate}
	...
	\item Es calcula el guanyador (puja mínima per subhastes a la baixa) de totes les subhastes una rere l'altra. En cas de trobar una puja condicionada sobre una subhasta que no s'ha executat es considera la puja com a bona i s'executa la subhasta com si fos una puja normal, a la vegada que es fa una \textit{anotació} marcant que aquesta puja depèn dels resultats futurs d'una altra subhasta futura. Quan l'altra subhasta s'executi, i obtinguem els resultats de qui ha sigut el guanyador d'aquesta, es revisita la subhasta depenent i, en cas que la puja condicional ja no sigui vàlida, aquesta subhasta és tornar a executar ignorant la puja invalidada. Aquestes invalidacions es propaguen sobre totes les subhastes deprenents.
