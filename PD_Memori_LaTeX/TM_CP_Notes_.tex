...
\begin{enumerate}
	\item \textbf{Simulated Annealing:} Algorisme  basat en el mètode de Montecarlo \textit{refinat} apropiat per \textbf{espais de cerca} grans. Té origen en una tècnica de l'industria metal·lúrgica i es sol aplicar en \textit{intel·ligència artificial}. Es troba implementat en Python en el paquet \textit{simanneal}, tot i que és relativament \textit{senzill} d'implementar.
	\item \textbf{Algorismes genètics:} La selecció d'algoritmes genètics, de forma molt simplificada, consisteix en l' estratègia  de davant un problema donat, i partint d'un nombre concret d'algorismes que hi donen una solució, trobar un algorisme \textit{combinació} dels altres amb o sense modificacions aleatòries o \textbf{mutacions} que aconsegueixi un resultat molt millor que els seus algoritmes de partida o \textit{algoritmes antecessors}, imitant així la \textit{selecció natural biològica}.
