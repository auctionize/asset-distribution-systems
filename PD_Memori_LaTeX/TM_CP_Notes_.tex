...
\item \begin{tabular}{|c|m{50mm}|}
	...
	\hline
	\textbf{Funció (B:4):}& \footnotesize $\begin{matrix} \\ f_4(x):=\left\lbrace 
	\begin{matrix}
...
Tot i que no és la \textit{filosofia del treball}. Això no ens resulta problemàtic, ja que només ens cal que la \textbf{solució} sigui \textit{bona}, entenent com a bon que sigui: \textbf{factible} , que consumeixi un temps \textit{raonable} en ser trobada i que sigui \textit{substancialment millor} que la del \textbf{model actual}. I això és pot comprovar amb fàcilment com farem en la subsecció de 3.5 Resultats
...
\vspace{3mm}
\normalsize
Concloem doncs que els models resultat
	\begin{figure}[!]
		\centering
...
\normalsize
Com hem explicat a l'apartat 3.3, els algoritmes genètics generen possibles solucions a bass de recombinar i mutar solucions ja existents. Aquests mecanismes abstractes, així com la codificació de la solució sobre la qual operen, admeten moltes possibles variacions, les quals s'han d'adaptar al problema per aconseguir el màxim rendiment.En la nostra implementació de l'algoritme genètic, hem optat per representar el fenotip (la codificació de cada solució generada) com una matriu de professors, on cada posició defineix quin professor es assignat a una assignatura diferent. La mutació es implemenlumna representasignció d'una assignatura aleatòria a un professor aleatori, per altra part la recombinació crea noves solucions a partir d'agafar dos solucions triades aleatoriameant entre les millors (la élit) i generant una nova solució que tingui part de les assignacions d'assigatures d'una i part de l'altra. Altres parametres, com la mida de la població, s'han triat 
A més a més, el nostre algoritme utilitza elitisme, de manera que les millors solucions són conservades entre iteracions de l'algorisme, portant a que 
\begin{figure}[ht]
	\centering
	\includegraphics[width=9cm]{progress_ga}
	\caption{ \small  \nehh{Albert has de ficar coses aquí6}. }
	\label{fio}
\end{figure}
