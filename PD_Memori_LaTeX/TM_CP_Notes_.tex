...
%Perquè càpiguen les coses a la capçalera 
...
%Serveix per recordar alguna cosa que s'ha de fer però en el moment fa mandra
...
%Serveix per fer el estil de capçalera
\usepackage{fancyhdr}
...
% Serveix per ficar lletres d'un alfabet 
...
%%%                                                              %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%{ \color{gray!3} Albert Acebrón i Marc Graells}
%Dades pel títol del nostre document
...
\hline
\\
\title{ \Large \textbf{Abstrac}}\\
\\
			\textit{Es parteix del cas concret de crear un model, que solucioni la problemàtica adjacent a l'assignació  de tasques docents entre el professorat del Departament de Matemàtiques de la Universitat. Comencem amb un primer anàlisis del problema abstracte. Seguidament fem un recull de tota la informació disponible sobre les particularitats del cas concret a resoldre. Elaborem i implementem un model basat en  la filosofia d'investigacions d'operacions que dóna solucions factibles que milloren les actuals. De forma recreativa s'apliquen algoritmes genètics per demostrar-ne la viabilitat. S'aborda la problemàtica des de l'òptica de les subhastes i és fan ambicioses construccions de nous tipus de subhasta elaborats per donar solució a la insatisfacció dels models de subhastes trobats en la literatura actual. S'ha fet un esforç destacable en facilitar la pedagogia darrere de les idees tractades, amb l'ús de tots els recursos visuals que hem tingut a l'abast i hem considerat oportuns.}	\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                 %%
%    #00        Introducció/comentari/\cdots       %%
%                                                 %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\\
	...
	\begin{enumerate}
		\item Primer es fa una ronda de repartiment amb el professorat que té preferència (perquè repeteixen una assignatura). Després es fa una segona ronda amb les matèries restants, tot i que sovint hi ha canvis d'últim moment. En aquesta segona ronda  és on es fa un tractament més personal i més precís. 
		...
		\begin{enumerate}
	\item  Cada subdepartament només pot fer assignatures del bloc que li pertoca. En el cas que es volgués impartir alguna matèria d'una altra \textit{branca de coneixement}, hauria de ser convidat pel subdepartament responsable.
...
En la nostra implementació dels algoritmes genètics, hem optat per representar el \textbf{fenotip} (la codificació de cada solució generada) com una matriu de professors, on cada posició defineix quin professor es assignat a cada assignatura diferent. La \textbf{mutació} es implementada com una assignació d'una assignatura aleatòria a un professor aleatori; per altra part la recombinació crea noves solucions a partir de dos solucions triades aleatòriament entre les millors (la \textbf{elit}), generant una nova solució que tingui part de les assignacions d'assignatures d'una i part de l'altra. Altres paràmetres, com la mida de la població, la probabilitat de recombinació o la de mutació, s'han triat de forme empírica a base d'executar l'algorisme i buscar les constants que donaven els millors resultats.

Comparant els resultats obtinguts en l'optimització en que hem utilitzat \textit{Simulated Annealing} i en la d'Algoritmes Genètics, és trivial veure que els resultats de SA són molt millors, ja que s'assoleixen assignacions amb diferències d'hores més ben distribuïdes. Això és així perquè hem executat els algoritmes durant poc temps (menys de 20 minuts) i el fet que les iteracions en GA són molt més lentes ha portat a que no s'aconseguís arribar a un mínim local.
...
\end{cases}
$$
Llavors considerant $f_j'$ en comptes de $f_j$ ja tindríem el problema solucionat, ja que llavors tota \textbf{subfunció personalitzable} està fitada superiorment i el cas anterior no és pot donar. A més dóna incentius per no fer servir aquest tipus d'estratègies, ja que llavors molt probablement la optimització de $F$ acabi donant una \textbf{alternativa} $a$ prou dolenta pel \textit{picaresc docent}.
\newpage
$$
\esp
$$
...
Tots tenim una idea intuïtiva de subhasta. Més enllà de les conegudes subhastes d'\textit{eBay}\footnote{Aquest tipus s'anomenen subhastes angleses, que són subhastes on totes les puges són públiques i la puja més alta guanya, pagant el preu d'aquesta mateixa puja. }  n'hi ha de molts altres tipus, que no només poden ser classificades per un únic criteri sinó per molts. Explicitem algunes d'aquestes classificacions.
...
Per tant una subhasta típica no seria pas una bona solució al problema de repartiment. No obstant hi ha un tipus de subhasta que no maximitza el benefici del \textit{venedor} sinó el dels \textit{participants}, és a dir, en maximitza \textbf{la utilitat} d'aquests últims. 
...
Si el professor té acumulat un saldo positiu (ha fet més hores del que li pertoquen en els últims 3 anys) se li sumen 5 Kiwis per cada 10 hores que tingui en el saldo : 
\begin{itemize}
...
Realitzarem una subhasta per blocs de matèries, cada professor omplirà les caselles de les matèries per les que vulgui pujar d'un pdf amb el valor en kiwis que està disposat  a pagar. El preu inicial de cada matèria és de 0 Kiwis, la puja mínima és de 1 Kiwi, i a partir d'aquí es pot augmentar el preu de 0.5 Kiwis en 0.5 Kiwis (és a dir, les puges vàlides per a una assignatura són: 1K, 1.5K, 2K, 2.5K, 3K,3.5K, etc.)

Un programa recollira els resultats i atorgarà cada matèria al major pujador. En cas d'empat, es quedarà la matèria aquell professor qui partís de més kiwis al començament. En cas de tornar a haver empat, la assignatura quedarà lliure amb el valor obtingut com a valor inicial per a la següent subhasta.

D'aquesta manera es realitzaran tres subhastes. (En la última hi haurà la possibilitat de pujar amb Kiwis negatius: els professors poden oferir-se a fer una assignatura a canvi de rebre fins a 5 kiwis a canvi (pujant per -5 Kiwis)).
...
c_1+c_2=1
$$
Cal triar doncs unes constants que reflecteixin la importància de cada una de les variables en la funció. Tenim en compte els següents fets per fer la tria:
\begin{itemize}
...
\begin{enumerate}
	\item Els professors \textit{entren totes les seves puges} (que poden estar condicionades sobre altres puges). No s'aplica \textbf{bloqueig de moneda} sobre les puges que pot realitzar un participant.
...
\textit{Per exemple: si un participant puja 10 per un item, fent la puja dependent en una altra puja de valor 1, serà possible eliminar les dos puges amb una única puja de 2 de manera que observem ineficiències ja que el sistema podia haver obtingut 11 monedes o 2 i ha triat 2.}
